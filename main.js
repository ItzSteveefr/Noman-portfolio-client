...
window.initCarouselLogic = function () {
  ...
  function updateCarouselClasses() {
    items.forEach((item, index) => {
      ...
      else if (index === (currentItemIndex + 1) % items.length)
        item.classList.add("item-right");
      else if (index === (currentItemIndex - 1 + items.length) % items.length)
  ...
  window.moveCarousel = function (direction) {
    currentItemIndex =
      (currentItemIndex + direction + items.length) % items.length;
